      ...
      <header className="bg-primary text-white px-3 py-3 flex items-center justify-between shadow-md z-10">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-semibold leading-tight">Sape Elite</h1>
              <p className="text-xs text-white/70">Commande #{typeof id === 'string' ? id.substring(0,6) : '...'}</p>
            </div>
          </div>
        </div>
          ...
          return (
            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-2xl px-3 py-2 shadow-sm relative ${
                isMe 
                  ? 'bg-[#DCF8C6] rounded-tr-sm text-gray-800' // Vert WhatsApp
